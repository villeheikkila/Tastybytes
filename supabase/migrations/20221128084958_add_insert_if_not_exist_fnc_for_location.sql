...
CREATE OR REPLACE FUNCTION public.fnc__get_location_insert_if_not_exist(p_name text, p_title text, p_latitude numeric, p_longitude numeric, p_country_code character)
...
begin
  ...
  if v_location is null then
    ...
    values (p_name, p_title, p_latitude, p_longitude, p_country_code, auth.uid())
    returning *
      into v_location;
  end if;
