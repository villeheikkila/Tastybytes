drop function if exists "public"."fnc__get_friends_by_username"(p_username text);

alter table "public"."companies" add column "description" text;

set check_function_bodies = off;

Drop FUNCTION if exists public.fnc__get_company_summary(p_company_id integer);
...
begin
  return query (select count(ci.id)                                                        total_check_ins,
                       ...
                       round(avg(ci.rating) filter ( where b.created_by = auth.uid() ), 2) current_user_average_rating
                from companies c
