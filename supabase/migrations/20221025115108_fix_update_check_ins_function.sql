...
CREATE OR REPLACE FUNCTION public.fnc__update_check_in(p_check_in_id bigint, p_product_id bigint, p_rating integer DEFAULT NULL::integer, p_review text DEFAULT NULL::text, p_manufacturer_id bigint DEFAULT NULL::bigint, p_serving_style_id bigint DEFAULT NULL::bigint, p_friend_ids uuid[] DEFAULT NULL::uuid[], p_flavor_ids bigint[] DEFAULT NULL::bigint[])
...
BEGIN
  ...
  if p_friend_ids is not null then
    with old_tags as (select profile_id id
         ...
         tags_to_be_removed as (select o.id
                                from old_tags o
                                       left join new_tags n on o.id = n.id
                                where n.id is null)
    delete
    from check_in_flavors
    where id in (select id from tags_to_be_removed);
