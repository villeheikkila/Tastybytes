...
CREATE OR REPLACE PROCEDURE public.fnc__migrate_data(IN p_created_by text)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
  _creator uuid;
BEGIN
  ...
  with all_products as (select mt.flavour     name,
                        ...
                        from migration_table mt
                               ...
                               left join categories c on c.name = trim(mt."category"))
  insert
  into products (name, description, "sub-brand_id", category_id, migration_id,
                 created_by)
  select name,
         description,
         "sub-brand_id",
  ...
  insert
  into check_ins (rating, product_id, migration_id, manufacturer_id, created_by)
  select rating, product_id, migration_id, manufacturer_id, _creator created_by
  from all_check_ins
  on conflict do nothing;
END
$procedure$
