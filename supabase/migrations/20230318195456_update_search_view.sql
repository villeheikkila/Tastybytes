...
create or replace view "public"."view__search_product_ratings" as  SELECT p.id,
    p.name,
    p.description,
    p.created_at,
    p.created_by,
...
drop view materialized_view__search_product_ratings;


CREATE MATERIALIZED VIEW materialized_view__search_product_ratings AS
SELECT * FROM view__search_product_ratings;


create function fnc__search_products(p_search_term text, p_only_non_checked_in boolean, p_category_name text DEFAULT NULL::text, p_subcategory_id bigint DEFAULT NULL::bigint) returns SETOF materialized_view__search_product_ratings
  language sql
as
$$
select pr.*
