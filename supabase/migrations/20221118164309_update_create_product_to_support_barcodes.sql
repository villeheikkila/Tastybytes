drop function if exists "public"."fnc__create_product"(p_name text, p_description text, p_category_id bigint, p_sub_category_ids bigint[], p_brand_id bigint, p_sub_brand_id bigint);
...
begin
  ...
  if p_barcode_code is not null and p_barcode_type is not null then
    ...
    values (product_id, p_barcode_code, p_barcode_type, auth.uid());
  end if;
