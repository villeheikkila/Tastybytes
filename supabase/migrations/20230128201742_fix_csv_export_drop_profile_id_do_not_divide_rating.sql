...
drop function if exists "public"."fnc__export_data"();

CREATE OR REPLACE FUNCTION public.fnc__export_data()
 RETURNS TABLE(category text, subcategory text, manufacturer text, brand_owner text, brand text, sub_brand text, name text, reviews text, ratings text, username text)
 LANGUAGE plpgsql
AS $function$
BEGIN
