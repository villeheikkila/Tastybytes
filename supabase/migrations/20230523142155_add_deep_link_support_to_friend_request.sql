...
CREATE OR REPLACE FUNCTION public.fnc__create_deeplink(p_type text, p_id text)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
begin
...
begin
  ...
  return jsonb_build_object('notification', jsonb_build_object('title', v_title, 'body', v_body), 'data', jsonb_build_object('link', fnc__create_deeplink('checkins', v_check_in_id::text)));
end;
$function$
;
...
begin
  ...
  select c.id from check_in_tagged_profiles ctp
         left join check_ins c on ctp.check_in_id = c.id into v_check_in_id;
...
begin
  ...
  select user_id_1 from friends f
    where f.id = p_friend_id
    into v_profile_id;

  return jsonb_build_object('notification', jsonb_build_object('title', v_title, 'body', v_body, 'data', jsonb_build_object('link', fnc__create_deeplink('profiles', v_profile_id::text))));
end;
$function$
;
